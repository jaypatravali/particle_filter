...
class State_Transition():
	...
	def log_transformations(self, logged_dict, seq):
		...
		if seq not in logged_dict:
			self.odom_compute( Vehicle_coords, bearing, self.pos[seq])	
		else:
			for coordinate in logged_dict[seq]:	[[23.201429407411435, -7.168813245752102]]
				bearing.append( math.atan2(coordinate[1],coordinate[0]))
				Landmark_Vehicle_odom = [coordinate[0]+ self.pos[seq][0], coordinate[1]+ self.pos[seq][1] , coordinate[2] + self.pos[seq][2] ]
				Vehicle_coords.append( Landmark_Vehicle_odom )
			self.odom_compute( Vehicle_coords, bearing, self.pos[seq])	
	...
	def cluster_transformation(self, points, points_disp, seq):
		...
		self.clusters.cluster_tracker(self.sensor_readings['odometry'], current_frame, self.pos[seq], seq)
